...
@Controller
@RequestMapping("/post")
//-> PostController 클래스의 모든 컨틀롤러 메서드의 매핑 주소는 "/post"로 시작.
...
//		log.debug("POST: create(title={}, content={}, author={})"
...
//	@GetMapping("/modify")
//	public void modify(@RequestParam(name="id") int id, Model model) {
//		log.debug("modify()");
//		
//		Post post = postService.read(id);
//		model.addAttribute("post", post);
//	}
	...
	@GetMapping("/delete")
	public String delete(@RequestParam(name="id") int id, @RequestParam(name="id") int postId) {
		log.debug("delete(id={})", id);
		
		// 포스트 아이디에 달려있는 댓글 먼저 삭제
	...
	public String update(PostUpdateDto dto) {
		...
		// 서비스 컴포넌트의 메서드를 호출해서 데이터 베이스 테이블 업데이트를 수행.
